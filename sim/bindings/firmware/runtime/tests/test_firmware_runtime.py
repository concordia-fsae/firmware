...
from sim.bindings.firmware.spi import (
    SpiTransaction,
)
from rig.simple import SimpleComponent, SimpleNodeRig
from sim.bindings.firmware.can.can import CanSignalComparison, _CanSignalWakeAbi
from rig import DataPath, DataflowWait, PeriodicDataPathProducer
from rig import RustRuntimeHost
from rig.model_fixtures import (
    BatchObservedModel,
    FakeNode,
...
def test_simple_node_routes_component_ingress_to_explicit_egress_datapath():
    ...
    assert cluster.comm.has_python_routes()


def test_simple_nodes_use_native_routes_for_typed_component_datapaths():
    path = DataPath.named(_RigInfraPath.SIMPLE, _RigInfraPath.SCALAR)
    pending = [12.5]
    received = []

    source_component = SimpleComponent()
    source_component.add_scalar_output(
        path,
        pending=lambda: len(pending),
        recv=lambda: pending.pop(0) if pending else None,
    )
    sink_component = SimpleComponent()
    sink_component.add_scalar_input(
        path,
        send=lambda value: received.append(value) or True,
    )

    cluster = FirmwareClusterRig(
        source=SimpleNodeRig(source_component),
        sink=SimpleNodeRig(sink_component),
    )
    cluster.run_for(1)

    assert received == [12.5]
    assert not cluster.comm.has_python_routes()
