...
import sim.bindings.firmware.runtime as runtime_python
...
from sim.models.catalog import ClusterCatalog, rig_node_name
from rig.datapath import (
    DataPath,
)
from sim.bindings.firmware.runtime import (
    PeripheralBinding,
    PeripheralInterface,
    FirmwareClusterRig,
    peripheral_datapath,
...
from rig.node_config import NodeConfig, SchedulerConfig
from rig import ClusterRig, ModelRig
...
def test_runtime_python_facade_exports_all_firmware_runtime_adapters():
    expected = {
        "ClusterCanComms",
        "ClusterComms",
        "ClusterDataRoutes",
    ...
    assert set(runtime_python.__all__) == expected
    assert all(hasattr(runtime_python, name) for name in expected)


def test_firmware_cluster_receives_generic_node_and_cluster_configuration():
    node_configuration = NodeConfig(
        scheduler=SchedulerConfig(period_ns=2_000_000),
    )
    node = ModelRig(configuration=node_configuration)
    cluster_configuration = ClusterConfig(
        scheduler=SchedulerConfig(period_ns=5_000_000),
    )
    ...
    assert node.configuration is node_configuration
    assert node._python_scheduler_callbacks().period_ns == 2_000_000
    assert cluster.configuration is cluster_configuration
    assert isinstance(cluster, ClusterRig)
...
def test_datapath_namespaces_and_peripheral_bindings_are_typed():
    ...
    can_path = can_datapath("veh")
    timer_path = peripheral_datapath(
        DataPath.named("timer"),
        PeripheralBinding(
...
def test_datapath_component_and_can_factories_cache_identity():
    ...
    assert can_datapath("veh") is can_datapath("veh")
