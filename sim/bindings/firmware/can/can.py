...
from rig.time import RunUntilTimeout, duration_to_ns


class _CanDataPathNamespace(Enum):
    BUS = auto()


@cache
def can_datapath(bus: object) -> DataPath:
    return DataPath.named(_CanDataPathNamespace.BUS, bus)


def is_can_datapath(path: DataPath) -> bool:
    return len(path.parts) == 3 and path.parts[1] == _CanDataPathNamespace.BUS
...
class CanSignalWake:
    ...
    def _register_callback(self, callback, runtime=None) -> None:
        ...
        if cluster is None or node_name is None:
            return
        runtime = runtime or cluster.runtime
        if runtime is None:
            return
        node_index = runtime.node_index(node_name)
        if node_index is None:
            return
        self._abi.source_node = node_index
...
class CanInterface:
    ...
    def latest_signal(
        ...
        *,
        ...
        node_name = self._model._cluster_node_name
        if cluster is not None and node_name is not None:
            raw_value = cluster.runtime.latest_can_signal(
                node_name,
                descriptor.bus,
                descriptor.id,
                signal,
    ...
    def _run_until_signal_cmp(
        ...
        *,
        ...
        wait = cluster.runtime.begin_can_signal_wait(
