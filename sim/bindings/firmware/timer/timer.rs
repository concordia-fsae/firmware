...
pub(super) type ClusterTimerCountFn = unsafe extern "C" fn(i32, i32) -> u32;
pub(super) type ClusterTimerRecvManyFn =
    unsafe extern "C" fn(i32, i32, *mut TimerChannelEvent, u32) -> u32;
pub(super) type ClusterTimerSendManyFn = unsafe extern "C" fn(*const TimerChannelEvent, u32) -> u32;

impl FirmwareBackend {
    pub(super) fn update_scalar_state_scale(&mut self, node: u32, route_id: u32, value: f32) {
        self.interfaces
            .timer
...
// Timer route registration and observation are part of the timer binding.
// The cluster runtime supplies only the generic runtime handle.
#[unsafe(no_mangle)]
pub extern "C" fn rig_cluster_add_timer_route(
    ...
    with_runtime(|runtime| {
        runtime.register_route(InterfaceRoute::Timer(ClusterTimerRoute {
            source_node,
            interface,
            port,
            channel,
...
pub extern "C" fn rig_cluster_latest_timer_event(
    ...
    let event = with_runtime(|runtime| {
        runtime
            .interfaces
...
#[unsafe(no_mangle)]
pub extern "C" fn rig_model_timer_send_duty(event: *const TimerChannelEvent) -> bool {
    if event.is_null() {
        return false;
    }
    push_duty_input(unsafe { *event })
...
#[unsafe(no_mangle)]
pub extern "C" fn rig_model_timer_send_frequency(event: *const TimerChannelEvent) -> bool {
    if event.is_null() {
        return false;
    }
    push_frequency_input(unsafe { *event })
...
#[unsafe(no_mangle)]
pub extern "C" fn rig_model_timer_send_capture(event: *const TimerCaptureEvent) -> bool {
    if event.is_null() {
        return false;
    }
    push_capture_input(unsafe { *event })
