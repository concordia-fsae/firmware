...
from sim.bindings.can.can import (
    CanBusDescriptor,
    CanEvent,
...
from rig.datapath import DataPathKey, DataPath, DataPathRecord, datapath_key
from sim.bindings.core.peripheral import PeripheralInterface, require_peripheral_binding
from rig.model import ComponentRig, ModelRig
from rig.cluster_config import ClusterConfig
from rig.cluster import ClusterRig, DataflowRoutes
...
from rig.scalar import ScalarEvent
from sim.bindings.spi.spi import SpiTransaction
from sim.bindings.timer.timer import TimerChannelEvent


PayloadT = TypeVar("PayloadT")


class ClusterDataRoutes(DataflowRoutes):
    """Firmware route adapter layered over Rig's generic dataflow routes."""

    def __init__(self, cluster: FirmwareClusterRig) -> None:
    ...
    def connect(
        ...
        *,
        ...
        super().connect(path, source_node=source_node, sink_node=sink_node)

    def reset(self) -> None:
        super().reset()
        self.clear_native_routes()
    ...
    def records(self, path: DataPath) -> tuple[DataPathRecord[object], ...]:
        ...
        return records if records else self._native_scalar_records(path)
    ...
    def latest_record(
        ...
        *,
        ...
        latest = super().latest_record(path, source_node=source_node)
        for record in self._native_scalar_records(path):
            if source_node is not None and record.source != source_node:
                continue
    ...
    def _native_scalar_records(
        ...
        for node_name in self._cluster._rig_nodes:
            ...
            if route_id is None:
                continue
            event = ScalarEvent()
            if self._cluster.runtime.latest_scalar_event(
                node_name,
                route_id,
                event,
            ):
...
class ClusterCanComms:
    ...
    def _connect_native_route(
        ...
        *,
        ...
        if not self._cluster.runtime.add_can_route(
            source_node=source_node,
    ...
    def latest_message(
        ...
        *,
        ...
        event = CanEvent()
        if self._cluster.runtime.latest_can_message(
            node,
    ...
    def latest_bus_event(
        ...
        event = CanEvent()
        if self._cluster.runtime.latest_can_bus_event(
            node,
            bus_descriptor.index,
            event,
        ):
...
class ClusterTimerComms(_TypedClusterComms[TimerChannelEvent]):
    ...
    def records(self, path: DataPath) -> tuple[DataPathRecord[TimerChannelEvent], ...]:
        records = super().records(path)
        if records or path.metadata is None:
            return records
    ...
    def latest_event(
        ...
        *,
        ...
        if node is not None and path.metadata is not None:
            ...
            if binding.interface in (
                ...
                if self._cluster.runtime.latest_timer_event(
                    node,
                    interface,
...
class FirmwareClusterRig(ClusterRig):
    def __init__(
        self,
        *,
    ...
    def _create_dataroutes(self) -> ClusterDataRoutes:
        return ClusterDataRoutes(self)

    def _initialize_backend(self) -> None:
        self._runtime: FirmwareRuntime | None = None
        self.comm = ClusterComms(self, self.dataroutes)

    @staticmethod
    def _reject_duplicate_shared_libraries(nodes: dict[str, ModelRig]) -> None:
        ...
        if duplicates:
            ...
            raise ValueError(
                "FirmwareClusterRig cannot instantiate the same Rust-backed controller shared object "
    ...
    def _reset_backend(self) -> None:
        ...
        if self.runtime is None:
            return
        self.runtime.reset()
        self._populate_runtime(self.runtime)

    def _add_backend_node(self, name: str, node: ModelRig) -> None:
        if self.runtime is not None:
            self.runtime.add_node(name, node, online=True)

    def _run_backend(self, duration_ns: int, step_ns: int) -> None:
        if self.runtime is None:
            raise RuntimeError("FirmwareClusterRig backend is not initialized")
        self.runtime.run_for(
            duration_ns,
            step_ns,
    ...
    def _set_backend_node_online(self, name: str, online: bool) -> None:
        if self.runtime is not None:
            self.runtime.set_node_online(name, online)

    def _create_runtime(self) -> FirmwareRuntime:
        hosts = tuple(
            node for node in self.nodes.values() if hasattr(node, "bind_symbol")
        )
        ...
        host = hosts[0] if hosts else RustRuntimeHost()
        runtime = FirmwareRuntime(
            host=host,
            route=self._route_from_runtime,
        )
        self._populate_runtime(runtime)
        return runtime

    def _populate_runtime(self, runtime: FirmwareRuntime) -> None:
        for name, node in self._rig_nodes.items():
    ...
    def _sync_elapsed_from_runtime(self, *, nodes: bool = True) -> None:
        if self.runtime is None:
            return
        self.elapsed_ns = self.runtime.elapsed_ns()
        if not nodes:
            return
        elapsed_by_node = self.runtime.node_elapsed_ns_values()
