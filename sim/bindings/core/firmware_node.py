...
from rig.artifacts import buck_output, load_shared_library, repo_root
...
from sim.bindings.core.firmware_cluster import ClusterCanComms
from rig.datapath import DataPath, DataPathKey, datapath_key
from rig.node_abi import ModelDataPathDescriptor
...
from rig.model import ModelRig, datapath_route_id
from rig.node_config import NodeConfig
from rig.scalar import (
    ScalarRouteEndpoint,
    ScalarSinkRouteEndpoint,
    ScalarStateSinkRouteEndpoint,
)
from rig.scheduler import RustSchedulerCallbacks
from sim.bindings.spi.spi import SpiInterface, SpiPeripheralInterface, SpiTransaction, SpiRouteEndpoint
from sim.bindings.timer.timer import (
    TimerCaptureEvent,
    TimerChannelEvent,
    TimerInterface,
...
class FirmwareNodeRig(ModelRig):
    ...
    def rust_datapath_route_abi(
        ...
        if scalar_state_sink_abi is not None:
            if self._cluster_rig is not None and self._cluster_node_name is not None:
                ...
                if not self._cluster_rig.runtime.add_scalar_state_sink(
        ...
        if native_scalar_output is not None:
            ...
            if self._cluster_rig is not None and self._cluster_node_name is not None:
                if not self._cluster_rig.runtime.add_timer_source(
                ...
                if not self._cluster_rig.runtime.add_timer_scaled_scalar_source(
                    node=self._cluster_node_name,
                    route_id=route_id,
    ...
    def add_timer_scaled_scalar_output(
        ...
        *,
        ...
        binding = require_peripheral_binding(timer_path)
    ...
    def _datapath_from_descriptor(
        self, descriptor: ModelDataPathDescriptor
    ) -> DataPath:
        if descriptor.interface == PeripheralInterface.TIMER_DUTY:
            return self.timer.duty_events(descriptor.port, descriptor.channel)
        if descriptor.interface == PeripheralInterface.TIMER_FREQUENCY:
            return self.timer.frequency_events(descriptor.port, descriptor.channel)
        if descriptor.interface == PeripheralInterface.SPI_TRANSACTION:
    ...
    def _configure_model_abi(self) -> None:
        ...
        self._datapath_descriptor = self._bind_symbol(
            ...
            [ctypes.c_uint32, ctypes.POINTER(ModelDataPathDescriptor)],
            ctypes.c_bool,
        )
    ...
    def _bind_model_symbol(
        ...
        return self.bind_symbol(f"{self.symbol_prefix}_{suffix}", argtypes, restype)
    ...
    def bind_symbol(
        ...
        """Bind a symbol for the Rig runtime or a firmware model extension."""
