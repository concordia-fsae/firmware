...
HW_StatusTypeDef_E HW_CAN_init(void)
{
    ...
    hcan[CAN_BUS_ASS].Instance = CAN2;
    ...
    _Static_assert(((COUNTOF(CANRX_VEH_unpackList) + ((4U - COUNTOF(CANRX_VEH_unpackList) % 4U) % 4U)) / 4) + (COUNTOF(CANRX_VEH_unpackListExtID) / 2U) +
                   ((COUNTOF(CANRX_ASS_unpackList) + ((4U - COUNTOF(CANRX_ASS_unpackList) % 4U) % 4U)) / 4) + (COUNTOF(CANRX_ASS_unpackListExtID) / 2U)
    ...
    for (uint32_t i = 0U; i < COUNTOF(CANRX_ASS_unpackList); i += 4U)
    {
        CAN_FilterTypeDef filt = { 0U };
        filt.FilterBank           = filterBank++;
        filt.FilterMode           = CAN_FILTERMODE_IDLIST;
        filt.FilterScale          = CAN_FILTERSCALE_16BIT;
        // All filters are shifted left 5 bits
        filt.FilterIdHigh = CANRX_ASS_unpackList[i + 0U] << 5U;
        if ((i + 1U) < COUNTOF(CANRX_ASS_unpackList)) { filt.FilterIdLow = CANRX_ASS_unpackList[i + 1U] << 5U; }
        if ((i + 2U) < COUNTOF(CANRX_ASS_unpackList)) { filt.FilterMaskIdHigh = CANRX_ASS_unpackList[i + 2U] << 5U; }
        if ((i + 3U) < COUNTOF(CANRX_ASS_unpackList)) { filt.FilterMaskIdLow = CANRX_ASS_unpackList[i + 3U] << 5U; }
        filt.FilterFIFOAssignment = i % CAN_RX_FIFO_COUNT;
        filt.FilterActivation     = ENABLE;
        filt.SlaveStartFilterBank = (COUNTOF(CANRX_VEH_unpackList) + ((4U - COUNTOF(CANRX_VEH_unpackList) % 4U) % 4U)) / 4U;
        HAL_CAN_ConfigFilter(&hcan[CAN_BUS_ASS], &filt);
    }
    for (uint32_t i = 0U; i < COUNTOF(CANRX_ASS_unpackListExtID); i+= 2U)
    {
        CAN_FilterTypeDef filt = { 0U };
        filt.FilterBank           = filterBank++;
        filt.FilterMode           = CAN_FILTERMODE_IDLIST;
        filt.FilterScale          = CAN_FILTERSCALE_32BIT;
        // All filters are fucky - lookup RM0008 information
        filt.FilterIdHigh = (uint16_t)(CANRX_ASS_unpackListExtID[i + 0U] >> 13U);
        filt.FilterIdLow = (uint16_t)(CANRX_ASS_unpackListExtID[i + 0U] << 3U) | (0x01 << 2U);
        if ((i + 1U) < COUNTOF(CANRX_ASS_unpackListExtID)) {
            filt.FilterMaskIdHigh = (uint16_t)(CANRX_ASS_unpackListExtID[i + 1U] >> 13U);
            filt.FilterMaskIdLow = (uint16_t)(CANRX_ASS_unpackListExtID[i + 1U] << 3U) | (0x01 << 2U);
        }
        filt.FilterFIFOAssignment = i % CAN_RX_FIFO_COUNT;
        filt.FilterActivation     = ENABLE;
        filt.SlaveStartFilterBank = (COUNTOF(CANRX_VEH_unpackList) + ((4U - COUNTOF(CANRX_VEH_unpackList) % 4U) % 4U)) / 4U;
    ...
    HAL_CAN_ActivateNotification(&hcan[CAN_BUS_ASS], CAN_ENABLED_INTERRUPTS);
...
void HAL_CAN_MspInit(CAN_HandleTypeDef* canHandle)
{
    ...
    else if (canHandle->Instance == hcan[CAN_BUS_ASS].Instance)
    {
