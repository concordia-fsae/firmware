...
HW_StatusTypeDef_E HW_SPI_init(void)
{
    ...
    if (LL_SPI_Init(HW_spi_devices[HW_SPI_DEV_BMS].handle, &hspi1) != SUCCESS)
    {
        Error_Handler();
    }

    // enable SPI
    LL_SPI_Enable(HW_spi_devices[HW_SPI_DEV_BMS].handle);
...
{
    LL_SPI_Disable(HW_spi_devices[HW_SPI_DEV_BMS].handle);
