...
static inline void set_BMSVoltageMin(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 53;  // 10 bits, 5mV precision, range [0-5115]mV
}

static inline void set_BMSVoltageMax(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 43;  // 10 bits, 5mV precision, range [0-5115]mV
}

static inline void set_ENVPackVoltage(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 36;  // 7 bits, 1 deg C precision
}

static inline void set_ENVTempMax(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 29;  // 7 bits, 1 deg C precision
}

static inline void set_BMSChargeLimit(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x1F) << 24;  // 5 bits, 1A precision
}

static inline void set_BMSDischargeLimit(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0xFF) << 16;  // 8 bits, 1A precision
}
...
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 54;// 10 bits, 5mv precisiion
}

static inline void set_ENVAvgTemp(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 47;          // 7 bits, 1 deg C precission
}

static inline void set_BMSSOCMin(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 10)) & 0x3FF) << 37; // 10 bits, 0.1% precision, range [0-102.3]%
}

static inline void set_BMSSOCAAvg(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 10)) & 0x3FF) << 27; // 10 bits, 0.1% precision, range [0-102.3]%
}

static inline void set_BMSSOCMax(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 10)) & 0x3FF) << 17; // 10 bits, 0.1% precision, range [0-102.3]%
}

static inline void set_CellTemp0(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 10;         // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp1(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 3;          // 7 bits, 1 deg C precision, range [0-127]deg C
}
...
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 57; // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp3(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 50; // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp4(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 43;  // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp5(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 36; // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp6(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 29; // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp7(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 22; // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp8(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 15; // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp9(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 8;  // 7 bits, 1 deg C precision, range [0-127]deg C
}

static inline void set_CellTemp10(CAN_data_T* message, int16_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 1;  // 7 bits, 1 deg C precision, range [0-127]deg C
}
...
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 54;   // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage1(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 44;   // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage2(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 34;   // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage3(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 24;   // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage4(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 14;   // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage5(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 4;    // 10 bits, 5mV precision, range [0-5120]mV
}
...
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 43;    // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage13(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 33;    // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage14(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 23;    // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_CellVoltage15(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value * 200)) & 0x3FF) << 13;    // 10 bits, 5mV precision, range [0-5120]mV
}

static inline void set_PackVoltage(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t)(value / 10)) & 0x1FFF) << 0;    // 13 bits, 10mV precision, range [0-81910]mV
}
...
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 28;   // 7 bits, 1 deg C precision, range [0-128]deg C
}

static inline void set_BoardTemp1(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 21;   // 7 bits, 1 deg C precision, range [0-128]deg C
}

static inline void set_MCUTemp(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 14;   // 7 bits, 1 deg C precision, range [0-128]deg C
}

static inline void set_BoardAmbientTemp(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x7F) << 7;   // 7 bits, 1 deg C precision, range [0-128]deg C
}

static inline void set_BoardRelativeHumidity(CAN_data_T* message, float32_t value)
{
    message->u64 |= (((uint64_t) value) & 0x07F); // 7 bits, 1% precision, range [0-128]%
}
...
{
    message->u64 |= ((uint64_t)((value ? 0x01 : 0U))) << 29;
}

static inline void set_CoolPercentage0(CAN_data_T* message, uint8_t value)
{
    message->u64 |= ((uint64_t)(value & 0x7F)) << 22;   // 7 bits, 1% precision, range [0-128]%
}

static inline void set_CoolState1(CAN_data_T* message, bool value)
{
    message->u64 |= ((uint64_t)((value ? 0x01 : 0U))) << 21; // I'm not sure if this is a 1 bit or 1 byte value. If it's 1 bit, this message can be 4 bytes instead of 6. Same with CoolState0
}

static inline void set_CoolPercentage1(CAN_data_T* message, uint8_t value)
{
    message->u64 |= ((uint64_t)(value & 0x7F)) << 14;   // 7 bits, 1% precision, range [0-128]%
}

static inline void set_FanRPM0(CAN_data_T* message, uint16_t value)
{
    message->u64 |= ((uint64_t)(value)) << 7;          // 7 bits, 1% precision, range [0-128]%
}

static inline void set_FanRPM1(CAN_data_T* message, uint16_t value)
{
    message->u64 |= ((uint64_t)(value)) << 0;          // 7 bits, 1% precision, range [0-128]%
}
