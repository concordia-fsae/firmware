...
#if FEATURE_IS_ENABLED(NVM_TASK) | FEATURE_IS_ENABLED(NVM_SWI)
extern void lib_nvm_save(void);
#if FEATURE_IS_ENABLED(NVM_TASK) & FEATURE_IS_ENABLED(NVM_SWI)
#error "Cannot have an NVM swi and task enabled simultaneously"
#endif
#endif
...
void RTOS_createResources(void)
{
...
#if FEATURE_IS_ENABLED(NVM_SWI)
    NVM_swi = SWI_create(RTOS_SWI_PRI_0, &lib_nvm_save);
#endif // NVM_SWI
    ...
    for (uint16_t i = 0U; i < NUM_ASYNC_TASKS; i++)
    {
        RTOS_taskDesc_t* const task = &AsyncTasks[i];
        task->handle = xTaskCreateStatic(&taskFxn,
                                            task->name,
                                            task->stackSize,
                                            task,
