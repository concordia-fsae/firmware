        ...
        try:
            ...
            if interface_type == "virtual":
                if "baudrate" in can_bus_def:
                    raise Exception(
                        "Virtual buses must not define a baudrate."
                    )
            elif "baudrate" not in can_bus_def:
                raise Exception(
                    "Physical buses must define a baudrate."
                )
            try:
...
def process_bridges(node: CanNode):
    ...
    for route in node.forwarding_routes:
        ...
        for msg_name in message_names:
            bridge_message_to_bus(node, route["source_bus"], route["dest_bus"], msg_name)
...
def main():
    ...
    if args.node:
        codegen(lookup, zip(args.node, args.codegen_dir), args.bridge_codegen)
