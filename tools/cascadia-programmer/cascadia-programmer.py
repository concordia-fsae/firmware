...
PARAMS: Dict[int, ParamSpec] = {
    ...
    11: ParamSpec(
        11, "Resolver PWM Delay Command", "Unsigned integer", False, 1.0, None, None
    ),
    ...
    23: ParamSpec(
        23,
        "Shudder Compensation Gain Control",
        "Unsigned integer",
        False,
        100.0,
        None,
    ...
    30: ParamSpec(
        30, "OBD2 Enable Command", "Unsigned integer", False, 1.0, None, None
    ),
    # ----------------------------
    ...
    151: ParamSpec(
        151, "Resolver PWM Delay", "Unsigned integer", False, 1.0, None, None
    ),
    ...
    149: ParamSpec(
        ...
        1.0,
        None,
        {0: "Simple on/off switch", 1: "Ignition switch (momentary START)"},
    ),
    170: ParamSpec(
        170, "Relay Output State", "Unsigned integer", False, 1.0, None, None
    ),
    ...
    204: ParamSpec(
        204, "Analog Output Function Select", "Unsigned integer", False, 1.0, None, None
    ),
    # CAN Configuration
    ...
    146: ParamSpec(
        146, "CAN Command Message Active", "Boolean", False, 1.0, None, None
    ),
    147: ParamSpec(
        147,
        "CAN Bit Rate",
    ...
    148: ParamSpec(
        148, "CAN Active Messages Lo Word", "Unsigned integer", False, 1.0, None, None
    ),
    158: ParamSpec(
        158, "CAN Diagnostic Data Transmit Active", "Boolean", False, 1.0, None, None
    ),
    159: ParamSpec(
        159, "CAN Inverter Enable Switch Active", "Boolean", False, 1.0, None, None
    ),
    171: ParamSpec(
        171,
    ...
    172: ParamSpec(
        172, "CAN Timeout", "Unsigned integer", False, (1.0 / 3.0), "ms", None
    ),  # counts of 3ms
    ...
    234: ParamSpec(
        ...
        1.0,
        None,
        {0: "Same as Master", 1: "Opposite of Master"},
    ),
    235: ParamSpec(
        235, "CAN Fast Msg Rate", "Unsigned integer", False, 1.0, "ms", None
    ),
    236: ParamSpec(
        236, "CAN Slow Msg Rate", "Unsigned integer", False, 1.0, "ms", None
    ),
    237: ParamSpec(
        237, "CAN Active Messages Hi Word", "Unsigned integer", False, 1.0, None, None
    ),
    # 2.3.4.3 Current
    ...
    103: ParamSpec(
        103, "DC Voltage Hysteresis", "High Voltage", False, 10.0, "V", None
    ),
    104: ParamSpec(
        104, "DC Under-voltage Limit", "High Voltage", False, 10.0, "V", None
    ),
    106: ParamSpec(106, "Vehicle Flux Command", "Flux", True, 100.0, "Wb", None),
    # 2.3.4.5 Temperature
    112: ParamSpec(
        112, "Inverter Over-Temperature", "Temperature", True, 10.0, "degC", None
    ),
    113: ParamSpec(
        113, "Motor Over-Temperature", "Temperature", True, 10.0, "degC", None
    ),
    114: ParamSpec(
        114, "Zero Torque Temperature", "Temperature", True, 10.0, "degC", None
    ),
    115: ParamSpec(
        115, "Full Torque Temperature", "Temperature", True, 10.0, "degC", None
    ),
    ...
    167: ParamSpec(
        167, "Klp Torque", "Low-Pass Filter Gain", False, 10000.0, None, None
    ),
    168: ParamSpec(168, "Torque Rate Limit", "Torque", False, 10.0, "Nm", None),
    # 2.3.4.8 Speed
    111: ParamSpec(
        111, "Motor Over-speed", "Angular velocity", False, 1.0, "rpm", None
    ),
    126: ParamSpec(
        126, "REGEN Fade Speed", "Angular velocity", False, 1.0, "rpm", None
    ),
    ...
    190: ParamSpec(
        190, "Shudder Filter Frequency", "Frequency", False, 10.0, "Hz", None
    ),
    191: ParamSpec(
        191, "Shudder Speed Fade", "Angular velocity", False, 1.0, "rpm", None
    ),
    192: ParamSpec(
        192, "Shudder Speed Low", "Angular velocity", False, 1.0, "rpm", None
    ),
    193: ParamSpec(
        193, "Shudder Speed High", "Angular velocity", False, 1.0, "rpm", None
    ),
    ...
    180: ParamSpec(
        ...
        1.0,
        None,
        {0: "Brake switch mode", 1: "Brake pot mode"},
    ),
    ...
    185: ParamSpec(
        185, "REGEN Ramp Period", "(Counts x 0.001) sec", False, 1.0, "ms", None
    ),
...
def read_param(
    ...
    def attempt_once() -> Tuple[int, Any]:
        flush_bus_rx(bus)
        bus.send(
            build_param_cmd(arbid_cmd, spec.addr, is_write=False, signed=spec.signed)
        )
...
def verify_param(
    ...
    raw, _ = read_param(
        bus, arbid_cmd, arbid_resp, spec, timeout, retries, retry_backoff
    )
    if raw != expected_raw:
        raise RuntimeError(
            f"verify mismatch addr={spec.addr}: wrote raw={expected_raw} read raw={raw}"
        )
    ...
    if spec.fmt.lower() == "boolean":
        ...
        if isinstance(value, str):
            ...
            if t in ("false", "no", "off", "0"):
                return 0
        raise ValueError(
            f"Boolean value not understood for addr={spec.addr}: {value!r}"
        )
...
def cmd_list(args) -> int:
    ...
    for s in specs:
        ...
        if s.enum_map:
            enum = " enum=" + ", ".join(
                f"{k}:{v}" for k, v in sorted(s.enum_map.items())
            )
        print(
            f"{s.addr:>4}  {s.name}  [{s.fmt}]  signed={s.signed} scale={s.scale}{unit}{enum}"
        )
    return 0
...
def cmd_read(args, bus: can.BusABC, arbid_cmd: int, arbid_resp: int) -> int:
    if args.addr is None:
        ...
        if spec is None:
            print(
                f"ERROR: address {args.addr} not in embedded registry", file=sys.stderr
            )
            return 2
    ...
    if spec.enum_map:
        ...
        print(
            f"READ {spec.addr} {spec.name}: raw={raw} -> {eng}{unit} (scale={spec.scale})"
        )
...
def cmd_write(args, bus: can.BusABC, arbid_cmd: int, arbid_resp: int) -> int:
    ...
    if args.enum is not None:
        if spec.enum_map is None:
            print(
                f"ERROR: addr={spec.addr} is not enum-like; use --value",
                file=sys.stderr,
            )
            return 2
    ...
    if spec.enum_map:
        print(
            f"WRITE {spec.addr} {spec.name}: raw={raw_value} ({eng_written}) -> success"
        )
    elif spec.fmt.lower() == "boolean":
        ...
        unit = f" {spec.unit}" if spec.unit else ""
        print(
            f"WRITE {spec.addr} {spec.name}: raw={raw_value} ({eng_written}{unit}) -> success"
        )
...
def cmd_flash_yaml(args, bus: can.BusABC, arbid_cmd: int, arbid_resp: int) -> int:
    ...
    if yaml_offset != args.id_offset:
        print(
            f"NOTE: using YAML id_offset={hex(yaml_offset)} (CLI was {hex(args.id_offset)})",
            file=sys.stderr,
        )
    ...
    for addr in addrs:
        ...
        if not has_value and not has_raw:
            print(
                f"ERROR: addr={addr} has neither 'value' nor 'raw' in YAML",
                file=sys.stderr,
            )
            failures += 1
            if not args.continue_on_error:
        ...
        try:
            ...
            print(
                f"WRITE {addr} {spec.name}: raw={raw_value} ({eng_preview}) -> success"
            )
            print("  VERIFY OK")
        except Exception as e:
            print(
                f"ERROR: addr={addr} ({spec.name}) write/verify failed: {e}",
                file=sys.stderr,
            )
            failures += 1
            if not args.continue_on_error:
...
def main() -> int:
    ap = argparse.ArgumentParser(
        description="EEPROM param read/write tool (2-byte data, manual units, no DBC)"
    )
    ...
    ap.add_argument(
        "--bitrate", type=int, default=None, help="Optional bitrate (backend-dependent)"
    )
    ap.add_argument(
        "--timeout", type=float, default=0.6, help="Response timeout seconds"
    )
    ap.add_argument(
        "--retries",
        type=int,
    ...
    ls.add_argument(
        "--eeprom-only", action="store_true", help="Only show EEPROM range (100–499)"
    )
    ...
    wr.add_argument(
        "--value", type=float, help="Engineering value (tool applies scaling)"
    )
    ...
    dy.add_argument(
        "--eeprom-only", action="store_true", help="Only dump EEPROM range (100–499)"
    )
    ...
    fy.add_argument(
        "--continue-on-error",
        action="store_true",
        help="Attempt remaining params after an error",
    )
    fy.add_argument(
        "--skip-unknown",
        action="store_true",
        help="Skip YAML params not in the embedded registry",
    )
