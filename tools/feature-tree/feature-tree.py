...
def GenerateFeatures(selection_files: List[str], features_dict: List[dict] = list()):
        ...
        with open(selection_file, "r") as fd:
            ...
            try:
                if type(loaded_selections["featureDefs"]) is str:
                    ...
                    features_files += [
                        open(os.path.basename(loaded_selections["featureDefs"]))
                    ]
                elif type(loaded_selections["featureDefs"]) is list:
                    # TODO: Fix this once we remove scons support
                    features_files += [
                        open(os.path.basename(file))
                        for file in loaded_selections["featureDefs"]
                    ]
            except KeyError:
                raise Exception(
                    f"FeatureDefs: Definition file in {selection_file} not specified in 'featureDefs'."
                )
    ...
    for fd in features_files:
        ...
        if (
            "prefix" not in loaded_features["config"]
            or "description" not in loaded_features["config"]
        ):
            raise Exception(
                f"Feature: File {fd.name} doesn't contain a prefix or description."
            )
        defs_copy = loaded_features["defs"].copy()
        for loaded_feature in defs_copy:
            new_loaded_feature = (
                loaded_features["config"]["prefix"] + "_" + loaded_feature
            )
            loaded_features["defs"][new_loaded_feature] = loaded_features["defs"].pop(
                loaded_feature
            )
            if new_loaded_feature in features.keys():
                raise Exception(
                    f"Feature: {new_loaded_feature} in file {fd.name} already exists."
                )
        ...
        if "discreteValues" in loaded_features:
            for value in loaded_features["discreteValues"]:
                if value in discreteValues.keys():
                    raise Exception(
                        f"FeatureDef: {value} in file {fd.name} already exists in the feature tree."
                    )
            ...
            try:
                for feature in loaded_selections["features"]:
                    if feature not in features:
                        raise Exception(
                            f"FeatureDefs: Feature {feature} in {selection_file} not defined anywhere in Feature Tree."
                        )
                    if loaded_selections["features"][feature] is None:
                        ...
                        for values in discreteValues:
                            if (
                                loaded_selections["features"][feature]
                                in discreteValues[values]
                            ):
                                valid = True
                                loaded_selections["features"][feature] = (
                                    values
                        ...
                        if not valid:
                            raise Exception(
                                f"FeatureDefs: Requested value {loaded_selections['features'][feature]} not defined."
                            )
                ...
                raise Exception(
                    f"FeatureDefs: No features specified in {selection_file}"
                )
            ...
            if requested_feature not in features.keys():
                ...
                for values in discreteValues:
                    if features_dict[requested_feature] in discreteValues[values]:
                        ...
                        features_dict[requested_feature] = (
                            values + "_" + features_dict[requested_feature]
                        )
                ...
                if "requires" in features[feature]:
                    for requirement in features[feature]["requires"]:
                        if requirement not in feature_values.keys():
                            raise Exception(
                                f"FeatureDefs: Required feature {requirement} is not specified."
                            )
                        if (
                            feature_values[feature] is not None
                            and feature_values[feature] is not False
                            and feature_values[feature] != -1
                        ):
                            if (
                                feature_values[requirement] is None
                                or feature_values[requirement] == -1
                                or feature_values[requirement] is False
                            ):
                                raise Exception(
                                    f"FeatureDefs: Required feature {requirement} is disabled."
                                )
                if (
                    "restricts" in features[feature]
                    and feature_values[feature] is not False
                ):
                    for requirement in features[feature]["restricts"]:
                        if requirement not in features.keys():
                            raise Exception(
                                f"FeatureDefs: Restricted feature {requirement} is not specified."
                            )
                        if (
                            requirement in feature_values.keys()
                            and feature_values[requirement] is not False
                        ):
                            raise Exception(
                                f"FeatureDefs: Restricted feature {requirement} is enabled."
                            )
...
def GetOptions(variant_id: int, variants, kv: dict[str, str]):
    try:
        if variant_id not in variants:
            raise Exception(
                f"Invalid variant id. Supported IDs are {list(variants.keys())}"
            )
        override_options = (
            variants[variant_id]["options"] if variants[variant_id]["options"] else {}
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        render_generated_file(
            {"variants": options}, "BuildDefines_generated.h.mako", args.output
        )
        ...
        render_generated_file(
            {"features": features}, "FeatureDefines_generated.h.mako", args.output
        )
    except Exception as e:
        print(f"Error: {e}")
        exit(1)
